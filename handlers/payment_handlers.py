...
async def success_payment(msg: Message, session: DataInteraction, state: FSMContext, scheduler: AsyncIOScheduler, dialog_manager: DialogManager):
    data = await state.get_data()
    await msg.delete()
    await execute_rate(msg.from_user.id, msg.bot, session, scheduler, data)
    await session.update_statistics('buys_sum', data.get('price'))  # обновление общей выручки по базе данных
